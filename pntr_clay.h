...
// convert pntr_color to clay color
#define PNTR_CLAY_COLOR(color) ((Clay_Color){ (color).rgba.r, (color).rgba.g, (color).rgba.b, (color).rgba.a })

#ifdef PNTR_CLAY_IMPLEMENTATION

Clay_Dimensions pntr_clay_measure_text(Clay_StringSlice text, Clay_TextElementConfig *config, void *userData) {
...
void pntr_clay_render(pntr_image* destinationImage, Clay_RenderCommandArray renderCommands, pntr_font** fonts, float deltaTime, float mouse_x, float mouse_y, bool isMouseDown, float mouse_wheel_x, float mouse_wheel_y ) {
    Clay_SetMeasureTextFunction(pntr_clay_measure_text, fonts);
    ...
    for (uint32_t i = 0; i < renderCommands.length; i++) {
        ...
        switch (renderCommand->commandType) {
            ...
            case CLAY_RENDER_COMMAND_TYPE_TEXT: {
                ...
                pntr_draw_text_len(destinationImage, fonts[config->fontId], config->stringContents.chars, config->stringContents.length, boundingBox.x, boundingBox.y, textColor);
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_SCISSOR_START: {
                ...
                pntr_image_set_clip(destinationImage, (int)boundingBox.x, (int)boundingBox.y, (int)boundingBox.width, (int)boundingBox.height);
                break;
            }
            ...
            case CLAY_RENDER_COMMAND_TYPE_IMAGE: {
                ...
                if (sourceImage) {
                    ...
                    pntr_draw_image_scaled(destinationImage, sourceImage, (int)boundingBox.x, (int)boundingBox.y, scaleX, scaleY, 0.0f, 0.0f, PNTR_FILTER_BILINEAR);
                }
                break;
            }
